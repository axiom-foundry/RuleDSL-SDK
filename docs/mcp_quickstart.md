...
- **The engine library** (`ruledsl_capi.dll` on Windows,
  ...
  `RuleDSL-<version>` for your platform, verify hashes first
  (`SHA256SUMS.txt` is authoritative — see `docs/distribution.md`), then
  use the library from `bin/`. The Python package is pure Python — the
  engine is never inside it.

- **A rule library**: a directory holding `manifest.json` plus the rule
  files it declares. This repository ships a ready example at `rules/`.
...
The server speaks MCP over **stdio**: it is meant to be spawned by an MCP
client (Claude Desktop, an agent runtime, or the smoke client below), not
used interactively.
...
`Tools/mcp_demo/smoke_client.py` is a dependency-free (stdlib-only)
JSON-RPC/stdio client that spawns the real server and asserts the v0
contract: handshake, closed 3-tool list, byte-identical decision records
for identical calls, and the reserved-field rejection. Exit code 0 = PASS.

```bash
python Tools/mcp_demo/smoke_client.py \
    --engine-lib /path/to/libruledsl_capi.so \
    --wrapper /path/to/RuleDSL-SDK/bindings/python
```

(`--rules` defaults to this repository's `rules/`.) CI runs it on every
PR (`verify-mcp-python` job) against the shipped release engine —
downloaded from Releases and hash-verified first — so the transport path
is covered end to end against the same binaries users run, not just at
handler level.
...
| error `server/1 SRV_UNKNOWN_RULE_ID` | rule id not in the manifest (files on disk are invisible unless declared) | check `list_rules`; add the rule to `rules/manifest.json` |
