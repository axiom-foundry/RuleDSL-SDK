# RuleDSL v1.0 Lexical Core Annex
...
- [SYN-0010] Comment syntax in v1.0 SHALL be limited to the compatibility surface `#...<newline>`, `//...<newline>`, and `/*...*/`; no additional comment delimiters or forms are allowed.
...
- [SYN-0013] Unicode identifier characters outside the ASCII grammar SHALL be out of scope for v1.0 and SHALL be rejected as lexical errors.
- [DET-0024] Identifier lexing and keyword classification SHALL use ASCII rules only and SHALL NOT depend on locale.
- [SYN-0017] Core reserved keywords for lexical core v1.0 SHALL be exactly: `rule`, `priority`, `when`, `then`, `or`, `and`, `not`, `in`, `match`, `true`, `false`; these spellings are canonical documentation forms.
- [SYN-0018] Domain/profile-specific keywords SHALL be out of scope for lexical core v1.0 and SHALL be specified only by an active profile document.
Profile keywords for the v1.0 baseline are canonically defined by `docs/language/profile_decision_rules_v1_0.md`.
...
- [ERR-0028] Any source octet sequence that cannot be tokenized by v1.0 lexical rules SHALL cause compile-time failure with `ERR.COMPILE.INVALID_TOKEN`; identifier meaning SHALL remain stable across compatible versions.
...
- `ERR.COMPILE.INVALID_TOKEN`
  - Trigger: source text contains a token not recognized by v1.0 lexical rules.
  - Classification: compile-time error.
  - Stability: symbolic identifier meaning is stable across compatible versions.
